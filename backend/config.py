...
# ตัวคูณเวลาหน่วง (settle time) ทุกจุดที่ automation รอ CropWat ประมวลผลคำสั่ง —
# "normal" (1.0) คือค่าที่ทดสอบยืนยันมาตลอดในเครื่องพัฒนา ไม่เปลี่ยนพฤติกรรมของ
# ใครก็ตามที่ไม่ได้แตะการตั้งค่านี้ "ช้า" ไว้สำหรับคอมรุ่นเก่าที่ประมวลผลคำสั่งช้า
# (แฮงค์ถ้ารอไม่พอ) "เร็ว" สำหรับคอมแรงๆ ที่อยากลดเวลารันลงอีกหน่อย
SPEED_MULTIPLIERS: dict[str, float] = {"fast": 0.7, "normal": 1.0, "slow": 2.0}
...
class Settings(BaseModel):
    ...
    # v0.7.0: โหมดเบื้องหลังเก่า (background_mode + watcher เหวี่ยงหน้าต่าง +
    # click-shield) ถูกถอดออกทั้งหมด — ยืนยันจากการวิเคราะห์เครื่องจริงว่าเป็น
    # ต้นเหตุ CropWat crash ("Cannot make a visible window modal") ตอนสลับไฟล์
    # ข้ามเดือน/ปี แทนที่ด้วยโหมดเดสก์ท็อปซ่อน (ดู hidden_desktop_mode) — config
    # เก่าที่มี key "background_mode" ค้างอยู่โหลดได้ปกติ (pydantic ข้าม key แปลก)

    # ความเร็วในการส่งคำสั่งอัตโนมัติ (v0.5.14) — ยืนยันจากผู้ใช้: คอมรุ่นเก่า
    # ประมวลผลคำสั่งช้ากว่าเครื่องที่ใช้ทดสอบ ทำให้ CropWat แฮงค์ถ้าจังหวะรอ
    # (settle time ระหว่างคำสั่ง) สั้นเกินไป — "normal" คือค่าเดิมที่ทดสอบมาตลอด
    # (ไม่กระทบเครื่องที่ใช้ได้ปกติอยู่แล้ว) ดู SPEED_MULTIPLIERS ด้านล่าง
    speed_preset: str = "normal"

    # โหมดเดสก์ท็อปซ่อน (v0.6.0, default ตั้งแต่ v0.7.0) — รัน CropWat บน Win32
    # Desktop object แยกที่มองไม่เห็นบนจอ เมาส์/คีย์บอร์ดบนจอหลักไม่แตะเลย ไม่ต้อง
    # เหวี่ยงหน้าต่าง/shield ใดๆ (ตัดต้นตอ error "Cannot make a visible window
    # modal" ตอนสลับไฟล์ข้ามเดือน/ปี — ทดสอบสดยืนยันแล้ว) โปรแกรมเปิด CropWat ให้
    # เองในเดสก์ท็อปซ่อน (ต้องมี cropwat_exe_path — ถ้าว่าง desktop_session จะลอง
    # หา path มาตรฐานให้เอง) ปิดสวิตช์ = โหมดคลาสสิก (เห็น CropWat บนจอ ดึง
    ...
    # แค่ครั้งเดียวต่อปี (ใช้เดือนแรกสุดที่ทดลองปลูกในปีนั้น) แล้วใช้ไฟล์เดียวกัน
    # ซ้ำทุกวันปลูกในปีนั้น ทั้งที่วันปลูกเดือนหลังๆ อาจมีไฟล์เดือนตรงกว่าอยู่จริง —
    # ค่า default True = resolve ใหม่ตามเดือนจริงของทุกวันปลูก (แม่นกว่า) เปิดสวิตช์
    # ปิดได้เพื่อกลับไปพฤติกรรมเดิม (ทั้งปีใช้ไฟล์เดือนแรกสุดไฟล์เดียว)
    shift_year_per_candidate: bool = True
...
def word_path(settings: Settings) -> Path:
    return Path(settings.output_dir) / "Screenshots.docx"
