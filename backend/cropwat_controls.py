...
class ErrorDialogControls:
    ...
    # เวลาสูงสุดที่รอเช็คว่ามี dialog error เด้งขึ้นมาไหมหลังแต่ละ step (วินาที) —
    # v0.2.0 ลด 2.0→0.8, v0.5.7 ลด 0.8→0.3: จุดนี้ถูกเรียก ~4 ครั้งต่อ 1 วันปลูก
    # คือต้นทุนคงที่ก้อนใหญ่สุดที่เหลืออยู่ ตอนนี้ _raise_if_error_dialog เปลี่ยนเป็น
    # poll ถี่ๆ (ทุก 0.04 วิ) แล้วออกทันทีที่เจอ error — ค่านี้เป็นแค่ "เพดานรอ"
    # กรณีไม่มี error เท่านั้น Delphi เด้ง error dialog แทบทันทีที่ประมวลผลคำสั่ง
    # เมนูเสร็จ (synchronous ใน UI thread) 0.3 วิจึงเหลือเฟือ และถ้า error โผล่ช้า
    # กว่านั้นจริงๆ ก็ยังถูกจับที่จุดเช็คของ step ถัดไป + สัญญาณสำเร็จตามหลังอยู่ดี
    poll_timeout_seconds: float = 0.3
